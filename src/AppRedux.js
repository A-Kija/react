...
import BasicTrain from './components/BasicTrain';
import { selectShips, selectTrains } from './selects';



function App() { // <---- pagrindinis komponentas

    let currentTrains = selectTrains(basic.getState());

    basic.subscribe(()=> {
        if (currentTrains !== selectTrains(basic.getState())) {
            console.log('DER TRAINS!')
        }
        currentTrains = selectTrains(basic.getState());
    });
    ...
    return (
        <div className="App">
            ...
            <main>
                ...
                <button onClick={()=>basic.dispatch(addTrain({ type: 'disel train', long: '298' }))}>ADD DER TRAIN</button>
            </main>
